...
- **`/notebooklm` command (research toolkit):** the source-grounded parallel to `/research-deep`. Same vault-first scan logic, but instead of routing gap-fill queries to Perplexity + Grok over the open web, it bundles the top 12 vault notes as a single pasteable source and walks the user through running NotebookLM (notebooklm.google.com) grounded in their own materials. Two-phase: `--topic "..."` emits a bundle file + structured prompt template + JSON payload; `--save-response --topic "..." --slug "..."` reads the NotebookLM response from stdin and writes an AI-first note to `Research/NotebookLM/YYYY-MM-DD — <slug>.md` plus a propagation payload for `/obsidian-save`. Cost ~$0 (uses free NotebookLM access). Listed under `research` category; total command count is now 33 (was 32). The two research tracks (open-web via `/research-deep`, source-grounded via `/notebooklm`) are designed to run in parallel for high-value topics — contradictions across the two tracks are where the insight is.
